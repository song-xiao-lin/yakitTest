...
export const YaklangEngineWatchDog: React.FC<YaklangEngineWatchDogProps> = React.memo((props) => {
                ...
                switch (mode) {
                    case "local":
                        if (!props.credential.Password) {
                            ...
                            outputToWelcomeConsole("尝试启动本地进程（Random Local Password）")
                        }
    ...
    useDebounceEffect(
        () => {
            ...
            if (mode === "local" && props.credential.Password) {
                if (!startingUp.current) {
                    grpcStartLocalEngine({
                        ...
                        if (res.ok && res.status === "success") {
                            outputToWelcomeConsole("（Random Local Password）引擎启动成功！")
                            debugToPrintLog(`[INFO] （Random Local Password）本地新引擎进程启动成功`)
                            if (props.onKeepaliveShouldChange) {
                                props.onKeepaliveShouldChange(true)
                            }
                        } else {
                            ...
                            outputToWelcomeConsole(
                                "（Random Local Password）引擎启动失败:" + res.status + ":" + res.message
                            )
                            debugToPrintLog(
                                `[ERROR] （Random Local Password）本地新引擎进程启动失败: ${res.status + ":" + res.message}`
                            )
                        }
