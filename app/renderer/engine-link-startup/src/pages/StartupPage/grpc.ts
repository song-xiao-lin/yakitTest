...
import {CheckAllowSecretLocal} from "./components/LocalEngine/LocalEngineType"
...
export const grpcWriteEngineKeyToYakitProjects: APIFunc<string, boolean> = (version, hiddenError) => {
    return new Promise(async (resolve, reject) => {
        ipcRenderer
            .invoke(ipcEventPre + "write-engine-key-to-yakit-projects", version)
            .then(() => {
                resolve(true)
            })
            .catch((e: any) => {
                if (!hiddenError) yakitNotify("error", version + " 写入engine-sha256.txt失败：" + e)
                reject(e)
            })
    })
}
...
/** @name 引擎连接前校验 */
export const grpcCheckAllowSecretLocal: APIFunc<number, CheckAllowSecretLocal> = (port, hiddenError) => {
    return new Promise(async (resolve) => {
        ipcRenderer
            .invoke(ipcEventPre + "check-allow-secret-local-yaklang-engine", {
                port
            })
            .then((res) => {
                resolve(res)
...
/** @name 引擎启动 */
export const grpcStartLocalEngine: APIFunc<StartLocalEngine, CheckAllowSecretLocal> = (params, hiddenError) => {
    return new Promise(async (resolve) => {
