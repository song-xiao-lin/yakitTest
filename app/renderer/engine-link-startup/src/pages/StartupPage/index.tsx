...
import {LocalGVS} from "@/utils/yakitGV"
import {
...
import {GetConnectPort, getReleaseEditionName} from "@/utils/envfile"
...
export const StartupPage: React.FC = () => {
    ...
    const onDownloadedYaklang = useMemoizedFn((isOk: boolean) => {
        ...
        if (["installNetWork", "skipAgreement_Install", "old_version"].includes(getYakitStatus())) {
    ...
    const initializeEngine = useMemoizedFn(async () => {
        ...
        try {
            ...
            setCheckLog([`解压失败：${error}，请点击下载引擎继续使用...`])
            setYakitStatus(isInitLocalLink.current ? "installNetWork" : "skipAgreement_Install")
        } finally {
            setRestartLoading(false)
        }
    })
    ...
    const handleOperations = useMemoizedFn((type: YakitStatusType | YaklangEngineMode, extra?: TypeCallbackExtra) => {
        switch (type) {
            case "skipAgreement_Install": // 小风车重置引擎失败
